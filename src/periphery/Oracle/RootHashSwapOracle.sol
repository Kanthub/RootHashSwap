...
import "./RootHashSwapOracleStorage.sol";

/// @title RootHashSwapOracle
/// @notice A TWAP oracle based on RootHashSwap Pair (time-weighted average price)
contract RootHashSwapOracle is RootHashSwapOracleStorage {
    ...
    constructor(address _pair, uint256 _minTwapPeriod) {
        ...
        pair = IRootHashSwapPair(_pair);
        MIN_TWAP_PERIOD = _minTwapPeriod;

        address _token0 = pair.token0();
        address _token1 = pair.token1();
        if (_token0 == address(0) || _token1 == address(0)) {
            revert RootHashSwapOracle_InvalidPair();
        }
        token0 = _token0;
        token1 = _token1;
    ...
    /// @dev Typically called periodically by a keeper, frontend, or other contract
    function update() external {
        ...
        if (timeElapsed < MIN_TWAP_PERIOD) {
            revert RootHashSwapOracle_InsufficientElapsedTime();
        }
    ...
    function consult(address tokenIn, uint256 amountIn) external view returns (uint256 amountOut) {
        ...
        if (tokenIn == token0) {
            ...
            revert RootHashSwapOracle_InvalidToken();
        }
    }
