...
import "./RootHashSwapSlidingOracleStorage.sol";

/// @title RootHashSwap Sliding Window TWAP Oracle
/// @notice Computes TWAP using multiple historical observations (sliding window)
contract RootHashSwapSlidingOracle is RootHashSwapSlidingOracleStorage {
    ...
    function currentCumulativePrices(address pair) public view returns (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) {
        IRootHashSwapPair RootHashSwapPair = IRootHashSwapPair(pair);
        price0Cumulative = RootHashSwapPair.price0CumulativeLast();
        price1Cumulative = RootHashSwapPair.price1CumulativeLast();

        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast_) = RootHashSwapPair.getReserves();
    ...
    function consult(address tokenIn, uint256 amountIn, address tokenOut) external view returns (uint256 amountOut) {
        address pair = RootHashSwapLibrary.pairFor(factory, tokenIn, tokenOut);
        ...
        (address token0, ) = RootHashSwapLibrary.sortTokens(tokenIn, tokenOut);
